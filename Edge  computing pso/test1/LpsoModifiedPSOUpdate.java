 ...
 *
 */
public class LpsoModifiedPSOUpdate extends ParticleUpdate {
    ...
    LpsoModifiedPSOUpdate(ParticlePSO1 particle) {
        super(particle);
        this.obj = particle;
    }
    ...
    public void update(Swarm swarm, Particle particle) {
        ...
        for (int i = 0; i < Constants.NoOfTasks; i++) {
            ...
            v[i]=w2*v[i]+(2*Math.random()*(pbest[i]-x[i])+2*Math.random()*(gbest[i]-x[i]));
