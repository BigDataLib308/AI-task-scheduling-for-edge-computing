...
public class KPso1 {
    ...
    public KPso1(int[] tasklength, int[] outputfilesize, int[] mips, double[] execcost,
...
//		swarm.setParticleUpdate(new UpdationPSO(new ParticlePSO()));
        swarm.setParticleUpdate(new KpsoModifiedPSOUpdate(new ParticlePSO1())); //更新粒子 的速度和位置
        for (int i = 0; i < Constants.NoOfIterations; i++) {    //评估粒子
            swarm.evolve();

//                System.out.println("  KPSO Global best at iteration " + i + " :" + swarm.getBestFitness());
//            System.out.println( swarm.getBestFitness());

        }
        System.out.println("  KPSO The best fitness value is " + swarm.getBestFitness());
        System.out.println("\nKPSO Best makespan: " + ff.calcMakespan(swarm.getBestParticle().getBestPosition()));
