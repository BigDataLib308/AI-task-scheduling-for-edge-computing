 ...
 *
 */
public class LKpsoModifiedPSOUpdate extends ParticleUpdate {
    ...
    LKpsoModifiedPSOUpdate(ParticlePSO1 particle) {
        super(particle);
        this.obj = particle;
    }
    ...
    public void update(Swarm swarm, Particle particle) {
         ...
         */
          double w2 = wmax+(wmin-wmax)*Math.log10(1+10*it/Constants.NoOfIterations);

          double k=2/Math.abs(2+4*c-Math.sqrt(2*Math.pow(c,2)-4*c));
        ...
        for (int i = 0; i < Constants.NoOfTasks; i++) {
			 ...
			 */
            // 1.
//			v[i] = 0.729844 * v[i] + 2 * Math.random() * (pbest[i] - x[i])
//					+ 2 * Math.random() * (gbest[i] - x[i]);
            // 2.
            v[i]=w2*v[i]+k*(2*Math.random()*(pbest[i]-x[i])+2*Math.random()*(gbest[i]-x[i]));
