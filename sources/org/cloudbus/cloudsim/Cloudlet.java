...
public class Cloudlet {
	...
	private String newline;
	private StringBuffer history;
	private final List<Cloudlet.Resource> resList;
	private int index;
	private int classType;
	private int netToS;
	public static final int CREATED = 0;
	...
	public Cloudlet(int cloudletId, long cloudletLength, int pesNumber, long cloudletFileSize, long cloudletOutputSize, UtilizationModel utilizationModelCpu, UtilizationModel utilizationModelRam, UtilizationModel utilizationModelBw, boolean record, List<String> fileList) {
	...
	public Cloudlet(int cloudletId, long cloudletLength, int pesNumber, long cloudletFileSize, long cloudletOutputSize, UtilizationModel utilizationModelCpu, UtilizationModel utilizationModelRam, UtilizationModel utilizationModelBw, List<String> fileList) {
		this(cloudletId, cloudletLength, pesNumber, cloudletFileSize, cloudletOutputSize, utilizationModelCpu, utilizationModelRam, utilizationModelBw, false);
		this.vmId = -1;
		this.accumulatedBwCost = 0.0D;
		this.costPerBw = 0.0D;
	...
	public Cloudlet(int cloudletId, long cloudletLength, int pesNumber, long cloudletFileSize, long cloudletOutputSize, UtilizationModel utilizationModelCpu, UtilizationModel utilizationModelRam, UtilizationModel utilizationModelBw, boolean record) {
	...
	public double getWaitingTime() {
		if (this.index == -1) {
			...
			double subTime = this.resList.get(this.index).submissionTime;
	...
	public long getCloudletFinishedSoFar() {
		if (this.index == -1) {
			...
			long finish = ((Cloudlet.Resource)this.resList.get(this.index)).finishedSoFar;
	...
	public void setCloudletFinishedSoFar(long length) {
		if (!((double)length < 0.0D) && this.index >= 0) {
			Cloudlet.Resource res = (Cloudlet.Resource)this.resList.get(this.index);
			res.finishedSoFar = length;
			if (this.record) {
	...
	public int getResourceId() {
		return this.index == -1 ? -1 : ((Cloudlet.Resource)this.resList.get(this.index)).resourceId;
	}
	...
	public void setResourceParameter(int resourceID, double cost) {
		Cloudlet.Resource res = new Cloudlet.Resource();
		res.resourceId = resourceID;
		res.costPerSec = cost;
		res.resourceName = CloudSim.getEntityName(resourceID);
		...
		if (this.index == -1 && this.record) {
			...
			int id = ((Cloudlet.Resource)this.resList.get(this.index)).resourceId;
			String name = ((Cloudlet.Resource)this.resList.get(this.index)).resourceName;
	...
	public double getSubmissionTime() {
		return this.index == -1 ? 0.0D : ((Cloudlet.Resource)this.resList.get(this.index)).submissionTime;
	}
	...
	public double getCostPerSec() {
		return this.index == -1 ? 0.0D : ((Cloudlet.Resource)this.resList.get(this.index)).costPerSec;
	}

	public double getWallClockTime() {
		return this.index == -1 ? 0.0D : ((Cloudlet.Resource)this.resList.get(this.index)).wallClockTime;
	}
	...
	public String[] getAllResourceName() {
		...
		if (size > 0) {
			...
			for(int i = 0; i < size; ++i) {
				data[i] = ((Cloudlet.Resource)this.resList.get(i)).resourceName;
			}
		}
	...
	public int[] getAllResourceId() {
		...
		if (size > 0) {
			...
			for(int i = 0; i < size; ++i) {
				data[i] = ((Cloudlet.Resource)this.resList.get(i)).resourceId;
			}
		}
	...
	public double getActualCPUTime(int resId) {
		Cloudlet.Resource resource = this.getResourceById(resId);
	...
	public Cloudlet.Resource getResourceById(int resourceId) {
		Iterator i$ = this.resList.iterator();

		Cloudlet.Resource resource;
		do {
			if (!i$.hasNext()) {
				return null;
			}

			resource = (Cloudlet.Resource)i$.next();
